...
class CoinSpawn(Spawn):
    ...
    def generate(self, window, image, robot, **kwargs):
        if self.spawn_counter >= self.spawn_interval:
            self.spawn_interval = max(self.min_spawn_interval, self.spawn_interval - 10)
            self.spawn_counter = 0

            self.spawn_position = (self.spawn_position * 3 + robot.x + 150) % (window.get_width() - image.get_width())

            return self._create_object(self.spawn_position, 0, image, window, **kwargs)
        return None
...
class MonsterSpawn(Spawn):
    ...
    def generate(self, window, image, robot, **kwargs):
        if self.spawn_counter >= self.spawn_interval:
            self.spawn_interval = max(self.min_spawn_interval, self.spawn_interval - 20)
            self.spawn_counter = 0
...
class DoorSpawn(Spawn):
    ...
    def generate(self, window, image, robot, **kwargs):
        if self.spawn_counter >= self.spawn_interval:
            self.spawn_interval = max(self.min_spawn_interval, self.spawn_interval - 200)
            self.spawn_counter = 0
